...
set :scm, "git"
set :repository, "git://www.dbx.tryphon.priv/audiobank"
...
namespace :deploy do
  ...
  task :symlink_shared, :except => { :no_release => true }  do
    ...
    run "ln -nfs #{shared_path}/casts #{release_path}/media/casts"
    run "ln -nfs #{shared_path}/upload #{release_path}/media/upload"
    run "ln -nfs #{shared_path}/playlists #{release_path}/public/cache"

    # FIXME remove media/cast
    run "ln -nfs #{shared_path}/casts #{release_path}/media/cast"
  end
end
