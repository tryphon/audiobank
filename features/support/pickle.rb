...
# Create Potimart::Name model via "name" (instead of "potimart name")
class Pickle::Adapter::PotimartOrm < Pickle::Adapter::Orm
  def self.factories
    model_classes.map{|k| new(k)}
  end

  def initialize(klass)
    @klass, @name = klass, klass.name.underscore.gsub('/','_').gsub(/^potimart_/,"")
  end
end
...
Pickle.configure do |config|
  config.adapters = [:factory_girl, :potimart_orm, :orm]
end
